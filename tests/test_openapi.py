...
async def test_swagger_file_url(aiohttp_client):
    ...
    app = web.Application()
    setup_swagger(
        app, ui_version=3, swagger_from_file=TESTS_PATH + "/data/example_swagger.yaml"
    )
...
async def test_swagger_info(aiohttp_client, swagger_info):
    ...
    description = "Test Custom Swagger"
    setup_swagger(
        app, ui_version=3, swagger_url="/api/v1/doc", swagger_info=swagger_info
    )
...
async def test_data_defs(aiohttp_client):
    ...
    assert (
        result["components"]["schemas"]["User"]["properties"]["permissions"]["items"][
            "$ref"
        ]
        is not None
    )
    assert (
        result["components"]["schemas"]["Permission"]["properties"][
            "permission_param_3"
        ]["default"]
        is not None
    )
